...
CONSTANTS.GAME_CONFIG.LEVELS = [...];
CONSTANTS.CANVAS_CONFIG = { width: 800, height: 600 };
...
ModalManager.setupModal('settings', false); // Settings
ModalManager.setupModal('level', true, data); // Level info
...
### **Canvas Management**
```javascript
// ✅ OBBLIGATORIO: Usare pattern standard per canvas
const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');
ctx.clearRect(0, 0, canvas.width, canvas.height);

// ✅ OBBLIGATORIO: Gestire resize canvas
...
```javascript
// ✅ OBBLIGATORIO: Gestire touch events correttamente
canvas.addEventListener('touchstart', handleTouchStart, { passive: false });
canvas.addEventListener('touchmove', handleTouchMove, { passive: false });
canvas.addEventListener('touchend', handleTouchEnd, { passive: false });

// ✅ OBBLIGATORIO: Prevenire scroll durante touch
...
### **❌ ERRORE**: Setup manuale touch events
```javascript
